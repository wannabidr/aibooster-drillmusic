...
let midiListener: MidiListener | null = null;
let aiService: AIService | null = null;
let currentSessionId: number | bigint | null = null;
...
// Select MIDI device
ipcMain.handle('select-midi-device', async (_event, deviceId: number) => {
  try {
    if (!midiListener) {
...
// Set log file path
ipcMain.handle('set-log-path', async (_event, logPath: string) => {
  try {
    if (!logWatcher) {
