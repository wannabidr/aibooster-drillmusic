...
describe('AIService', () => {
  ...
  beforeEach(() => {
    ...
    aiService = new AIService(mockWindow as BrowserWindow);
  });
  ...
  describe('추천 요청 (Recommendation Request)', () => {
    it('추천 요청이 올바른 형식을 가져야 함', async () => {
      ...
      await expect(
        aiService.getRecommendation(request)
      ).rejects.toThrow('Python process not running');
    });
    ...
    it('필수 파라미터가 없으면 에러를 반환해야 함', async () => {
      ...
      await expect(
        aiService.getRecommendation(invalidRequest)
      ).rejects.toThrow();
    });
